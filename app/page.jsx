  ...
  const financialModel = useMemo(() => {
    ...
    return rawMonths.map((r, idx) => {
      ...
      if (idx === 0) {
        ...
        posWtdChurnForAttached = posWtdChurn;
      } else {
      ...
      // Core LTV at POS retention - for Attached calculation
      // Uses Core's L3M ARPA and GM, but POS's L3M churn rate
      const coreLTVAtPosRetention = posWtdChurnForAttached > 0 ? (coreAvgARPA * coreAvgGM) / posWtdChurnForAttached : 0;
      ...
      return {
        ...
        coreLTV, posLTV, totalLTV, coreLTVAtPosRetention,
            ...
            <div className="overflow-auto max-h-[calc(100vh-140px)]">
              <table className="w-full text-xs">
                ...
                <tbody className="divide-y divide-gray-100">
                  ...
                  <Row label="LTV ($) - Attached" coreValue={m => '-'} posValue={m => '-'} totalValue={m => fmtC(m.coreLTVAtPosRetention + m.posLTV, 0)} showSubRows={false} isEOP={true} isSum={false} />
                  ...
                  <Row label="LTV/CAC - Attached" coreValue={m => '-'} posValue={m => '-'} totalValue={m => { const attachedLTV = m.coreLTVAtPosRetention + m.posLTV; const attachedCAC = m.coreCAC + m.posCAC; return attachedCAC > 0 && isFinite(attachedLTV / attachedCAC) ? `${(attachedLTV / attachedCAC).toFixed(1)}x` : '-'; }} showSubRows={false} isEOP={true} isSum={false} />
              ...
              <div className="mb-6">
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    ...
                    <tr className="border-b"><td className="py-1.5 px-2">LTV ($)</td><td className="text-right py-1.5 px-2">{fmtC((financialModel[2]?.coreLTVAtPosRetention || 0) + (financialModel[2]?.posLTV || 0), 0)}</td><td className="text-right py-1.5 px-2">{fmtC((financialModel[5]?.coreLTVAtPosRetention || 0) + (financialModel[5]?.posLTV || 0), 0)}</td><td className="text-right py-1.5 px-2">{fmtC((financialModel[8]?.coreLTVAtPosRetention || 0) + (financialModel[8]?.posLTV || 0), 0)}</td><td className="text-right py-1.5 px-2">{fmtC((financialModel[11]?.coreLTVAtPosRetention || 0) + (financialModel[11]?.posLTV || 0), 0)}</td></tr>
                    ...
                    <tr className="border-b"><td className="py-1.5 px-2">LTV/CAC</td><td className="text-right py-1.5 px-2">{(() => { const ltv = (financialModel[2]?.coreLTVAtPosRetention || 0) + (financialModel[2]?.posLTV || 0); const cac = (financialModel[2]?.coreCAC || 0) + (financialModel[2]?.posCAC || 0); return cac > 0 ? `${(ltv/cac).toFixed(1)}x` : '-'; })()}</td><td className="text-right py-1.5 px-2">{(() => { const ltv = (financialModel[5]?.coreLTVAtPosRetention || 0) + (financialModel[5]?.posLTV || 0); const cac = (financialModel[5]?.coreCAC || 0) + (financialModel[5]?.posCAC || 0); return cac > 0 ? `${(ltv/cac).toFixed(1)}x` : '-'; })()}</td><td className="text-right py-1.5 px-2">{(() => { const ltv = (financialModel[8]?.coreLTVAtPosRetention || 0) + (financialModel[8]?.posLTV || 0); const cac = (financialModel[8]?.coreCAC || 0) + (financialModel[8]?.posCAC || 0); return cac > 0 ? `${(ltv/cac).toFixed(1)}x` : '-'; })()}</td><td className="text-right py-1.5 px-2">{(() => { const ltv = (financialModel[11]?.coreLTVAtPosRetention || 0) + (financialModel[11]?.posLTV || 0); const cac = (financialModel[11]?.coreCAC || 0) + (financialModel[11]?.posCAC || 0); return cac > 0 ? `${(ltv/cac).toFixed(1)}x` : '-'; })()}</td></tr>
